...
func main() {
    ...
    // ただし、a_i が b_i の親であることが保証されています。(1 ≦ i ≦ N-1)
    var tree: [Node] = [Node](repeating: Node(parent: [], children: []), count: n)
    for _ in 1..<n {
        let itemsG = readLine()!.trimmingCharacters(in: .whitespacesAndNewlines).split(separator: " ")
        let p = Int(itemsG[0])!
        let c = Int(itemsG[1])!
        tree[p-1].children.append(c)
        tree[c-1].parent.append(p)
    }
    // ルート
    tree[r-1].parent.removeAll()
    // 要素をソート
    for i in 0..<n {
        tree[i].parent.sort { $0 < $1 }
