...
class route {
    ...
    public function __construct(position $p, int $c, $r) {
        $this->pos = $p;
        $this->cost = $c;
        $this->route = $r;
    }
    ...
    public function getKey() {
        // 判定には位置を使用
        return ($this->pos->x * 2**16 + $this->pos->y);
    }
}
...
function dijkstra(position $s, position $g, board $b) {
    ...
    $closed = array();
    // ゴールまでの経路
    $itineraries = array_fill(0, $b->width, null);
    // スタート
    $startRoute = new route($s, $b->getCost($s), null);
    ...
    while (!$open->isEmpty()) {
        ...
        foreach($MOVABLE_DIRECTION as $move) {
            ...
            if ($b->isWithinRange($nextPos)) {
               ...
               $nextRoute = new route($nextPos, $nextCost, $now);
...
// outputData は結果を出力する
// i $r[] : 経路
function outputData(array $itineraries) {
    foreach ($itineraries as $r) {
        echo $r->cost."\n";
    }
}
...
function outputRoute(array $itineraries, board $b) {
    foreach ($itineraries as $r) {
        echo "--\n";
        echo $r->cost."\n";
        $here = $r;
