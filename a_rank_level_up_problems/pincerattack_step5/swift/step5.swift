/*
これはpaizaラーニングの「Aランクレベルアップメニュー」の「リバーシの操作」に
swiftでチャレンジしたコードです。
...
struct World {
    ...
    // 座標が範囲内か判定
    func isInRange(y: Int, x: Int) -> Bool {
        return (0 <= y && y < self.H && 0 <= x && x < self.W)
    }
    // 裏返す
    mutating func take(py: Int, px: Int) {
        ...
        for a in able {
            ...
            var tx = px + a.X
            // マップの範囲内チェック
            while self.isInRange(y: ty, x: tx) {
                // print("\(ty), \(tx), \(self.S[ty][tx])")
                // 陣取りチェック
                if self.S[ty][tx] == "*" {
                    // ここから置いた位置まで置く
                    var oy = ty - a.Y
